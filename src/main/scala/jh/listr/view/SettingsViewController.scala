...
class SettingsViewController(
	...
	private def settingsInitialize(): Unit = {
		...
		ap_font.getSelectionModel.select(App.selectedFontIndex)
	}
	...
	private def saveSettings(check: Int): Unit = {
		if (check == 0) {
			...
			App.selectedFontIndex = ap_font.getSelectionModel.getSelectedIndex
			App.changeFont(ap_font.getSelectionModel.getSelectedItem)
			//hides the save buttons
			ap_hbox.visible = false
		}
